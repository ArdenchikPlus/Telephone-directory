...
def _load_and_normalize_image(path):
    ...
    1) ImageOps.exif_transpose — применяет EXIF-тег ориентации. Фото с
       ...
       без ошибки). ИСПРАВЛЕНО: у части фото (отредактированных сторонними
       программами, скриншотов и т.п.) EXIF-блок бывает повреждён и
       exif_transpose может бросить исключение — раньше это приводило к
       полному отказу загрузить фото. Теперь такие ошибки просто
       игнорируются, и фото используется как есть, без поворота.
    ...
    try:
        ...
        # Повреждённый/нестандартный EXIF-блок не должен блокировать
        # загрузку фото целиком — просто используем изображение как есть.
        pass
